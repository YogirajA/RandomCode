...
var frames = [];
var getFrameDetails = function (img) {
    var chainGunFrame = frames[img];
    var frame = chainGunFrame.frame;
    var cx = -Math.ceil(frame.w / 2);
    var cy = -Math.ceil(frame.h / 2);
    var drawObj = {
        "id": img,
        "x": frame.x,
        "y": frame.y,
...
SpriteSheetClass = Class.extend({
    ...
    parseAtlasDefinition: function (atlasJSON) {
        ...
        frames = JSON.parse(atlasJSON).frames;
        
        frames = JSON.parse(atlasJSON).frames;

        for (var frame in frames) {
            var chaingun = getFrameDetails(frame);
            this.defSprite(chaingun.id, chaingun.x, chaingun.y, chaingun.w, chaingun.h, chaingun.cx, chaingun.cy);
